...
if ( ! class_exists( 'YITH_POS_Assets' ) ) {
	...
	class YITH_POS_Assets {
		...
		public function enqueue_admin_scripts() {
			...
			if ( yith_pos_is_admin_page( 'shop_order' ) ) {
				global $post_id;
				...
				if ( $order ) {
					...
					foreach ( $coupon_items as $item ) {
						if ( yith_pos_is_discount_coupon_code( $item->get_code() ) ) {
							$pos_discount_coupons[ $item->get_code() ] = implode(
								' - ',
								array_filter(
									array(
										__( 'Discount', 'yith-point-of-sale-for-woocommerce' ),
										$item->get_meta( 'coupon_data' )['description'] ?? '',
									)
								)
							);
						}
			...
			if ( yith_pos_is_admin_page( 'panel', 'dashboard' ) && yith_pos_is_wc_admin_enabled() ) {
