...
class YITH_POS_Register_Session_Data_Store implements YITH_POS_Object_Data_Store_Interface {
	...
	public static function query( $args = array() ) {
		...
		if ( false !== $args['store_id'] ) {
			$where_clauses[] = $wpdb->prepare( 'register_sessions.store_id = %d', absint( $args['store_id'] ) );
		}

		if ( false !== $args['register_id'] ) {
			$where_clauses[] = $wpdb->prepare( 'register_sessions.register_id = %d', absint( $args['register_id'] ) );
		}
